...
- **Email verification (since 2026-08-18): app-level, not Supabase.** Supabase confirm-email stays OFF. New signups must enter a 6-digit code (sent via Resend, domain bewerbungski.com verified, EU region) before AI endpoints work (403 email_unverified). Existing accounts were backfilled as verified once (column-existence guard in migrate.ts). RESEND_API_KEY is set in Replit secrets AND as Railway service variable.
