...
function normalize(raw: any) {
  ...
  return {
    ...
    skills: arr(raw?.skills).map((s: any) => ({
      name: str(s?.name, 80),
      level: typeof s?.level === "number" && s.level >= 0 && s.level <= 100 ? Math.round(s.level) : null,
    })).filter(s => s.name),
    languages: arr(raw?.languages).map((l: any) => ({
      language: str(l?.language, 60), level: str(l?.level, 30),
    })).filter(l => l.language),
    ...(() => {
      const sc = raw?.school ?? {};
      ...
      if (!school.type && !school.name && schoolEntries.length) {
        ...
        school = { type: e.degree, name: e.institution, city: e.city, year: e.end.slice(0, 4) };
      }
      const ja = raw?.jobad ?? {};
  ...
  try {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        temperature: 0,
        system:
          'Du extrahierst Lebenslaufdaten aus kopiertem LinkedIn-Profiltext. Antworte AUSSCHLIESSLICH mit validem JSON (kein Markdown, keine Erklärungen) in exakt dieser Struktur: {"personal":{"firstName":"","lastName":"","title":"","email":"","phone":"","address":"","zip":"","city":"","linkedin":"","website":"","summary":""},"experience":[{"company":"","city":"","position":"","start":"JJJJ-MM","end":"JJJJ-MM","current":false,"description":""}],"education":[{"institution":"","city":"","degree":"","field":"","grade":"","start":"JJJJ-MM","end":"JJJJ-MM"}],"skills":[{"name":"","level":null}],"languages":[{"language":"","level":""}],"school":{"type":"","name":"","city":"","year":"JJJJ"},"jobad":{"title":"","company":"","address":"","description":""}}. Regeln: Wenn der Text eine angestrebte Stelle/Bewerbung erwähnt (z.B. "ich bewerbe mich als X bei Y"), fülle "jobad" (title=Stelle, company=Unternehmen, address=Adresse des Unternehmens falls genannt, description=Details zur Stelle); sonst alle jobad-Felder leer lassen. Der allgemeinbildende Schulabschluss (Hauptschulabschluss, Realschulabschluss/Mittlere Reife, Abitur, Fachabitur o.ä.) gehört in "school" (type=Abschlussart, name=Schulname, year=Abschlussjahr) und NICHT in "education"; "education" ist nur für Berufsausbildung/Studium/Weiterbildung. Eine ausdrücklich genannte Berufsausbildung/Lehre gehört als education-Eintrag hinein. Fehlende Felder als leerer String bzw. bei unbekanntem Skill-Level als null. NIEMALS Daten erfinden oder aus Berufsbezeichnungen typische Kenntnisse, Tätigkeiten oder Qualifikationen ableiten. Skills, Sprachen und deren Niveaus nur übernehmen, wenn sie ausdrücklich genannt sind. Daten im Format JJJJ-MM (bei nur Jahr: JJJJ-01). Bei aktueller Stelle current=true und end="". Formulierungen dürfen sprachlich professionalisiert werden, ihre Tatsachenbedeutung darf sich aber nicht ändern. Behalte die Originalsprache der Inhalte bei.',
  ...
  try {
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        temperature: 0,
        system:
          'Ein Bewerber beschreibt seinen Werdegang in eigenen Worten (formlos, umgangssprachlich, unvollständig). Extrahiere und strukturiere die Lebenslaufdaten. Antworte AUSSCHLIESSLICH mit validem JSON (kein Markdown, keine Erklärungen) in exakt dieser Struktur: {"personal":{"firstName":"","lastName":"","title":"","email":"","phone":"","address":"","zip":"","city":"","linkedin":"","website":"","summary":""},"experience":[{"company":"","city":"","position":"","start":"JJJJ-MM","end":"JJJJ-MM","current":false,"description":""}],"education":[{"institution":"","city":"","degree":"","field":"","grade":"","start":"JJJJ-MM","end":"JJJJ-MM"}],"skills":[{"name":"","level":null}],"languages":[{"language":"","level":""}],"school":{"type":"","name":"","city":"","year":"JJJJ"},"jobad":{"title":"","company":"","address":"","description":""}}. Regeln: Wenn der Text eine angestrebte Stelle/Bewerbung erwähnt (z.B. "ich bewerbe mich als X bei Y" oder "ich möchte mich als X bewerben"), fülle "jobad" (title=Stelle, company=Unternehmen, address=Adresse des Unternehmens falls genannt, description=Details zur Stelle); sonst alle jobad-Felder leer lassen. Der allgemeinbildende Schulabschluss (Hauptschulabschluss, Realschulabschluss/Mittlere Reife, Abitur, Fachabitur o.ä.) gehört in "school" (type=Abschlussart, name=Schulname, year=Abschlussjahr) und NICHT in "education"; "education" ist nur für Berufsausbildung/Studium/Weiterbildung. Eine ausdrücklich genannte Berufsausbildung/Lehre gehört als education-Eintrag hinein. Fehlende Felder als leerer String bzw. bei unbekanntem Skill-Level als null — NIEMALS Daten erfinden oder aus einem Beruf typische Kenntnisse, Tätigkeiten oder Qualifikationen ableiten. Ungefähre Zeitangaben ("vor 5 Jahren", "seit 2020") in JJJJ-MM umrechnen (heute ist ' + new Date().toISOString().slice(0, 7) + '). Bei nur Jahr: JJJJ-01. Bei aktueller Stelle current=true und end="". Formulierungen dürfen sprachlich professionalisiert werden, ihre Tatsachenbedeutung darf sich aber nicht ändern. Skills, Sprachen und deren Niveaus nur übernehmen, wenn sie ausdrücklich im Text stehen. Behalte die Sprache des Bewerbertextes bei.',
