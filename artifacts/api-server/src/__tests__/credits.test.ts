 ...
 * - Quota: limit = 1 free + purchased credits; correct error codes at the limit
...
describe("quota: 1 free + credits", () => {
  ...
  it("free user at 1 document must upgrade before generating again", async () => {
    state.docCount = 1;
    const res = await postGenerate();
  ...
  it("premium user at 31 (1 + 30) → premium_limit_reached", async () => {
    state.profile.isPremium = true;
    state.profile.credits = 30;
    state.docCount = 31;
    const res = await postGenerate();
  ...
  it("after buying a second package (60 credits), 31 documents is no longer the limit", async () => {
    state.profile.isPremium = true;
...
describe("legacy premium migration semantics", () => {
  it("legacy premium profile (is_premium, credits backfilled to 30 (legacy)) can still generate up to 31", async () => {
