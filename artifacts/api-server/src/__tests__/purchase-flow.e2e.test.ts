 ...
 *   5. Stripe webhook checkout.session.completed → is_premium=true, credits=30
 *   6. /me shows limit 33; generating works again
 *   7. Saves documents up to 33 total → generate → 403 premium_limit_reached
 *
          ...
          return {
            where() {
              if (table === profilesTable) {
                ...
                if (patch.credits !== undefined) state.profile.credits += 30; // sql`credits + 30`
              }
...
describe("E2E: 3 gratis → Kauf → 30 weitere → Limit 33", () => {
  ...
  it("step 5: webhook fulfillment unlocks premium with +30 credits", async () => {
    const res = await webhook("evt_e2e_1");
    expect(res.status).toBe(200);
    const me = await getMe();
    expect(me.body).toMatchObject({ is_premium: true, credits: 30, document_limit: 33 });
  });
  ...
  it("step 7: at 33 documents, generation is blocked with premium_limit_reached", async () => {
    // Fill up to the premium limit (3 already saved)
    for (let n = state.docs.length + 1; n <= 33; n++) {
      const save = await saveDocument(n);
      expect(save.status).toBe(201);
    }
    const me = await getMe();
    expect(me.body.documents_count).toBe(33);
